...
public sealed class HaDiscovery
{
    ...
    public IEnumerable<(string Topic, string Payload)> DiscoveryMessages()
    {
        ...
        yield return Sensor("control_state", "Control state", template: "{{ value_json.state }}",
            description:
        ...
        yield return Sensor("charger_status", "Charger status", template: "{{ value_json.charger_status }}",
            description:
        ...
        yield return Sensor("solar_power", "Solar power", template: "{{ value_json.solar_w }}",
        ...
        yield return Sensor("surplus", "Solar surplus (PV minus house)", template: "{{ value_json.surplus_w }}",
            description:
        ...
        yield return Sensor("ev_power", "EV charging power", template: "{{ value_json.ev_power_w }}",
            description: "Power the EV charger reports drawing right now - the car's actual consumption, not a setpoint.",
            unit: "W", deviceClass: "power", stateClass: "measurement");

        yield return Sensor("ev_current", "EV charging current (measured)", template: "{{ value_json.ev_current_a }}",
            description:
        ...
        yield return Sensor("target_current", "Target charging current (commanded)", template: "{{ value_json.target_a }}",
            description:
        ...
        yield return Sensor("active_current", "Active charging current (charger setpoint)", template: "{{ value_json.active_a }}",
            description:
        ...
        yield return Sensor("battery_soc", "Battery SOC", template: "{{ value_json.soc }}",
            description:
        ...
        yield return Sensor("battery_power", "Battery power (+ charging / - discharging)", template: "{{ value_json.battery_w }}",
            description:
        ...
        yield return Sensor("grid_power", "Grid power (+ import / - export)", template: "{{ value_json.grid_w }}",
            description:
        ...
        yield return Sensor("day_outlook", "Day outlook", template: "{{ value_json.outlook }}",
            description:
        ...
        yield return Sensor("plan_state", "Plan state", template: "{{ value_json.plan_reason }}",
            description: "One line on why the day plan says what it says - the same explanation the log carries.",
            icon: "mdi:text-box-outline");

        yield return Sensor("charge_window", "Charge window", template: "{{ value_json.window }}",
            description:
        ...
        yield return Sensor("ev_budget", "EV energy budget", template: "{{ value_json.ev_budget_kwh }}",
            description:
        ...
        yield return Sensor("ev_expected_today", "EV energy expected today", template: "{{ value_json.ev_expected_kwh }}",
            description: "What the car can realistically expect to receive in total today, including what it has already taken.",
            unit: "kWh", deviceClass: "energy");

        yield return Sensor("shortfall", "Projected shortfall", template: "{{ value_json.shortfall_kwh }}",
            description:
        ...
        yield return Sensor("soc_floor", "Required SOC floor", template: "{{ value_json.soc_floor }}",
            description:
        ...
        yield return Sensor("forecast_remaining", "Forecast remaining today", template: "{{ value_json.forecast_remaining_kwh }}",
            description:
            "Forecast PV production still to come today, at the configured confidence band and already scaled by the "
            + "bias below.",
            unit: "kWh", deviceClass: "energy");

        yield return Sensor("tomorrow_forecast", "Tomorrow forecast", template: "{{ value_json.tomorrow_kwh }}",
            description:
            "Tomorrow's forecast production. Purely informational: context for whether a shortfall today is worth "
            + "waiting out.",
            unit: "kWh", deviceClass: "energy");

        yield return Sensor("forecast_accuracy", "Forecast accuracy", template: "{{ value_json.bias_percent }}",
            description:
        ...
        yield return Sensor("session_energy", "Session energy", template: "{{ value_json.session_kwh }}",
            description: "Energy delivered to the car since it was plugged in. Starts afresh when it is unplugged.",
            unit: "kWh", deviceClass: "energy");

        yield return Sensor("loaned_today", "Battery loaned today", template: "{{ value_json.loaned_kwh }}",
            description:
        ...
        yield return Sensor("loan_power", "Battery loan power", template: "{{ value_json.loan_w }}",
            description:
        ...
        yield return Number(DailyEvTargetNumber, "Daily EV target",
            description:
        ...
        yield return Number(SessionEnergyTargetNumber, "Session energy target",
            description:
        ...
        yield return Number(MinBatterySocNumber, "Minimum battery SOC",
            description:
        ...
        {
            ["name"] = "Controller charging the car",
            ["state_topic"] = StateTopic,

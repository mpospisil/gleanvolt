...
public sealed class HostShutdown : IServiceShutdown
{
    ...
    private void LogStopped()
    {
        if (_stopRequested)
        {
            _logger.LogInformation(
                "Gleanvolt stopped cleanly at the request of {Source}. Exiting with code "
        ...
        _logger.LogInformation(
            "Gleanvolt stopped cleanly after a termination signal. Exiting with code "
    ...
    public void RequestStop(string source)
    {
        ...
        _logger.LogWarning(
            ...
            + "'docker compose start gleanvolt-controller' on the Pi.",
            source);
